...
{
    DYNAMIC( uint8_t, encrypted, plain_length + crypto_box_MACBYTES ); // -C99
                          ...
                          const uint8_t *sendback_data, uint16_t length, const uint8_t *shared_encryption_key)
{
    ...
    DYNAMIC( uint8_t, data, 1 + nodes_length + length + 1 + crypto_box_PUBLICKEYBYTES+ crypto_box_NONCEBYTES + crypto_box_MACBYTES ); // -C99
                                      ...
                                      const uint8_t *nodes_data, uint16_t nodes_data_length)
{
    ...
    uint8_t packet[MAX_CRYPTO_REQUEST_SIZE];
    DYNAMIC( uint8_t, data, 1 + crypto_box_PUBLICKEYBYTES + nodes_data_length ); // -C99
