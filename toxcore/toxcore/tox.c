...
Tox *tox_new(const struct Tox_Options *options, TOX_ERR_NEW *error)
{
    ...
    m_options.client_caps = options->client_capabilities;
...
size_t tox_get_savedata_size(const Tox *tox, uint8_t save_friends)
{
    ...
    return messenger_size(m, save_friends);
}
...
void tox_get_savedata(const Tox *tox, uint8_t *savedata, uint8_t save_friends)
{
    if (savedata) {
        const Messenger *m = tox;
        messenger_save(m, savedata, save_friends);
    }
}
...
void tox_callback_cryptpacket_before_send(Tox *tox, tox_friend_cryptpacket_before_send_cb *callback)
{
    Messenger *m = tox;
    m_cryptpacket_before_send(m, callback);
}
...
bool tox_friend_send_lossless_packet2(Tox *tox, uint32_t friend_number, const uint8_t *data1, size_t length1,
                                      const uint8_t *data2, size_t length2)
{
    Messenger *m = tox;
    return 0 == send_custom_lossless_packet2(m, friend_number, data1, length1, data2, length2);
}
