                              ...
                              uint32_t length)
{
    ...
    //uint8_t temp[sizeof(theid) + length]; // C99
    size_t sizeof_temp = sizeof(uint8_t) * (sizeof(theid) + length); // -C99
    uint8_t* temp = _alloca( sizeof_temp ); // -C99
    theid = htonl(theid);
    memcpy(temp, &theid, sizeof(theid));
    memcpy(temp + sizeof(theid), message, length);
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_MESSAGE, temp, /*sizeof(temp)*/ sizeof_temp, 0);
}
                             ...
                             uint32_t length)
{
    ...
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_ACTION, temp, /*sizeof(temp)*/ sizeof_temp, 0);
}
...
static int handle_packet(void *object, int i, uint8_t *temp, uint16_t len)
{
    ...
    switch (packet_id) {
        ...
        case PACKET_ID_ACTION: {
            ...
            /* Make sure the NULL terminator is present. */
            //uint8_t action_terminated[action_length + 1]; // C99
            size_t sizeof_action_terminated = sizeof(uint8_t) * (action_length + 1); // -C99
            uint8_t* action_terminated = _alloca( sizeof_action_terminated ); // -C99
        ...
        case PACKET_ID_FILE_SENDREQUEST: {
            ...
            //uint8_t filename_terminated[data_length - 1 - sizeof(uint64_t) + 1]; // C99
            size_t sizeof_filename_terminated = sizeof(uint8_t) * (data_length - 1 - sizeof(uint64_t) + 1); // -C99
