...
{
    return (netcrypto->crypto_connections + crypt_connection_id)->connection_number_tcp;
}


IP_Port return_ip_port_connection_mutex(Net_Crypto *netcrypto, int crypt_connection_id)
{
    Crypto_Connection *conn = netcrypto->crypto_connections + crypt_connection_id;

    pthread_mutex_lock(&conn->mutex);
    IP_Port ipp = return_ip_port_connection(netcrypto, crypt_connection_id);
    pthread_mutex_unlock(&conn->mutex);
    return ipp;
}
